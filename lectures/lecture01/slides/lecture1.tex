...
% Slide 8: Theory-Practice Balance
...
\begin{tikzpicture}[scale=0.8]
    ...
    \draw[->,thick] (s0) to node[above] {$A_0$} (s1);
    \draw[->,thick] (s1) to node[above] {$A_1$} (s2);
    \draw[->,thick,dashed] (s2) to node[above] {...} (st);
    
    \draw[->,red] (s1) to[bend right=45] node[below] {$R_1$} (0.5,-1);
    \draw[->,red] (s2) to[bend right=45] node[below] {$R_2$} (3.5,-1);
\end{tikzpicture}
\end{center}
