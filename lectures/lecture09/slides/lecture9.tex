...
\begin{itemize}
    ...
    \item Actor-Critic architectures, A2C and A3C
...
\begin{itemize}
    ...
    \item Explain the difference between synchronous A2C and asynchronous A3C
\end{itemize}
...
% Slide 5: Actor-Critic Concept (Text Only)
\begin{frame}{Actor-Critic: Best of Both Worlds}
\begin{itemize}
    \item \textbf{Actor (Policy):}
    \begin{itemize}
        \item Learns policy $\pi_\theta(a|s)$
        \item Selects actions in each state
        \item Updated via policy gradients
    \end{itemize}

    \item \textbf{Critic (Value Function):}
    \begin{itemize}
        \item Learns value function $V_\phi(s)$
        \item Evaluates how good a state (or action) is
        \item Provides a learned baseline for the actor
    \end{itemize}

    \item \textbf{Key Idea:}
    \begin{itemize}
        \item Actor focuses on \emph{choosing actions}
        \item Critic focuses on \emph{evaluating} those actions via $V(s)$
        \item Combining them reduces variance while keeping policy gradient flexibility
    \end{itemize}
\end{itemize}
\end{frame}

% Slide 6: Actor-Critic Information Flow (Diagram Only)
\begin{frame}{Actor-Critic Information Flow}
\begin{center}
\begin{tikzpicture}[>=Latex]
    ...
    \draw[->] (critic) -| node[pos=0.85, below] {value / baseline} (actor);
    \draw[->] (env) -- ++(0,1.4) -| node[above left] {$s_{t+1}$} (state);
\end{tikzpicture}

\vspace{0.6em}

{\small Critic learns $V_\phi(s)$ as a baseline, which is used to form advantages for updating the actor.}
\end{center}
\end{frame}
...
\end{frame}
% --- New Frame: A3C ---
\begin{frame}{Asynchronous Advantage Actor-Critic (A3C)}
\textbf{Key Idea:} Multiple workers run in parallel and update a shared set of parameters asynchronously.

\textbf{Architecture:}
\begin{itemize}
    \item One global network (shared actor-critic parameters)
    \item Many worker agents, each with:
    \begin{itemize}
        \item Its own environment instance
        \item A local copy of the network
        \item Its own rollout horizon and gradients
    \end{itemize}
    \item Workers periodically push gradients to the global network and pull updated parameters
\end{itemize}

\textbf{Training Loop (per worker):}
\begin{enumerate}
    \item Sync local network parameters from the global network
    \item Run for $t_{\max}$ steps (or until termination) in its own environment
    \item Compute advantages and returns from the local trajectory
    \item Compute policy, value, and entropy losses
    \item Apply gradients to update the global parameters (asynchronously)
\end{enumerate}

\textbf{Original Motivation:} Fully CPU-based parallelism with Hogwild-style updates, no GPU required.
\end{frame}
% --- New Frame: A2C vs A3C ---
\begin{frame}{A2C vs A3C: Parallelism Patterns}
\textbf{Synchronous A2C:}
\begin{itemize}
    \item Collect rollouts from all $N$ environments in lockstep for $T$ steps
    \item Aggregate a single large batch and perform one update
    \item Easier to implement and debug
    \item Naturally suited to GPU training
\end{itemize}

\textbf{Asynchronous A3C:}
\begin{itemize}
    \item Each worker runs and updates independently
    \item No strict synchronization across workers
    \item Reduces correlation between trajectories
    \item Exploits many CPU cores without heavy coordination
\end{itemize}

\textbf{Modern Practice:}
\begin{itemize}
    \item A2C (and PPO) are more common in GPU-based implementations
    \item A3C is historically important and still useful on CPU clusters
    \item Conceptually, A2C is a synchronized, batched version of A3C
\end{itemize}
\end{frame}
...
\begin{center}
\includegraphics[width=0.5\linewidth]{figures/value_estimate_random_policy.png}
\newline
...
\begin{center}
\includegraphics[width=0.6\linewidth]{figures/gae_lambda_profiles.png}
\end{center}
...
\begin{center}
\includegraphics[width=0.6\linewidth]{figures/a2c_vectorized_learning_curve.png}
\end{center}
\end{frame}
...
\begin{center}
\includegraphics[width=0.6\linewidth]{figures/a2c_entropy_sweep.png}
\end{center}
...
\begin{center}
\includegraphics[width=0.6\linewidth]{figures/a2c_integrated_learning.png}
\end{center}
